...
def write_csv_text(path, text):
    with codecs.open(path, "w", "utf-8-sig") as handle:
        handle.write(text)
...
def write_hydropower_template(path):
    ...
    if not os.path.exists(path):
        write_csv_text(path, template)
        return
    with codecs.open(path, "r", "utf-8-sig") as handle:
        existing = handle.read().strip().replace(u"\r\n", u"\n")
    if existing in [
...
def init_project(init_root, output_prefix, arcmap_template_mxd):
    ...
    write_text(
        ...
        u"   水电站位置请填写自动生成的 倾泻点.csv 模板；ID 默认为 1，NAME 默认为 水电站位置，只需要填写 Longitude 和 Latitude。\n"
        u"\n"
    ...
    write_text(
        ...
        u"ID 默认为 1，NAME 默认为 水电站位置，只需要填写 Longitude 和 Latitude 两列；经纬度使用十进制度。\n"
